...
export default function TvDetails() {
  ...
  useEffect(() => {
    ...
    return () => {
      dispatch(removetv()); // Clean up the effect
    };
  }, [id]);
  ...
  return info ? (
    <div
      ...
      className="relative w-full h-[190vh] px-[10%] text-white "
    >
      {/* Part 1 Navbar */}
      ...
      {/* Part 3 Seasons and Episodes */}
      <hr className="border-1 border-zinc-400 mt-5 mb-2" />
      <h2 className="text-3xl text-white font-semibold ">Seasons</h2>
      <div className="w-[100%] flex gap-3 overflow-y-hidden p-5">
        {info.detail.seasons.map((season, index) => (
            <div key={index} className="h-[35vh] w-[30vh] mr-[2%] flex shrink-0 flex-col 
            items-center bg-[#1f1e24] rounded-lg shadow-lg hover:scale-105 transition-transform
            duration-300">
              <img 
                src={`https://image.tmdb.org/t/p/original/${season.poster_path}`}
                alt={season.name}
                className="h-[80%] w-full object-cover rounded shadow-[8px_17px_38px_2px_rgba(0,0,0,0.5)]"
              />
              <div className="text-white p-3 h-[20%] overflow-y-auto">
                <h1 className="text-lg text-white font-semibold tracking-tight leading-5">
                {season.name}
              </h1>
              </div>
            </div>
